...
export default function B2BAdmin() {
  ...
  return (
    <div className="space-y-8">
      <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#215b94]">Relacionamento empresarial</p><h1 className="mt-1 text-2xl font-semibold">Gestão Comercial</h1><p className="text-sm text-muted-foreground">Empresas, tabelas, cotações, pedidos e importações Ideal Prime.</p></div>
      ...
      <section><h2 className="mb-3 font-semibold">Tabelas Comerciais</h2><div className="flex max-w-lg gap-2"><Input value={name} onChange={(event) => setName(event.target.value)} /><Button onClick={() => createList.mutate({ name, isDefault: !(lists.data?.length) })}>Criar tabela</Button></div><div className="mt-3 space-y-1 text-sm">{lists.data?.map((priceList: any) => <div key={priceList.id}>{priceList.name} {priceList.is_default ? "· padrão" : ""} · versão {priceList.latest_version}</div>)}</div><p className="mt-2 text-xs text-muted-foreground">Importação CSV/XLSX permanece em /api/b2b/import com tabela, modo e arquivo autenticados.</p></section>
      <section><h2 className="mb-3 flex items-center gap-2 font-semibold"><FileText className="h-5 w-5 text-[#215b94]" /> Cotações Comerciais</h2><div className="space-y-2">{quotes.data?.map((quote: any) => <div className="rounded-xl border border-border bg-card p-4" key={quote.id}><div className="flex flex-col justify-between gap-2 md:flex-row"><div><strong>{quote.quote_number} · {quote.trade_name || quote.legal_name}</strong><div className="text-xs text-muted-foreground">{quote.buyer_name} · {new Date(quote.created_at).toLocaleString("pt-BR")}</div></div><strong className="text-[#067c52]">{money(Number(quote.total_cents || 0))}</strong></div><div className="mt-3 flex flex-wrap items-center gap-2"><span className="rounded-full bg-[#e8f1fb] px-2.5 py-1 text-xs font-semibold text-[#215b94]">{quote.status}</span>{quote.status === "PENDING" && <><Button size="sm" onClick={() => transitionQuote.mutate({ id: quote.id, action: "APPROVE" })}>Aprovar</Button><Button size="sm" variant="outline" onClick={() => transitionQuote.mutate({ id: quote.id, action: "REJECT" })}>Recusar</Button></>}{quote.status === "APPROVED" && <Button size="sm" variant="outline" onClick={() => transitionQuote.mutate({ id: quote.id, action: "CANCEL" })}>Cancelar</Button>}</div></div>)}{!quotes.data?.length && <div className="rounded-xl border border-dashed p-6 text-center text-sm text-muted-foreground">Nenhuma cotação comercial recebida.</div>}</div></section>
      ...
      <section><h2 className="mb-2 font-semibold">Importações de Catálogo</h2>{imports.data?.map((job: any) => <div key={job.id} className="text-sm">#{job.id} · {job.mode} · {job.status} · {job.price_list_name}</div>)}</section>
    </div>
  );
}
