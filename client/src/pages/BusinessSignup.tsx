...
export default function BusinessSignup(){
 ...
 const signup=trpc.b2b.signup.useMutation({onSuccess:()=>setMessage("Cadastro recebido. Sua empresa está aguardando aprovação."),onError:e=>setMessage(e.message)});
 const set=(k:string,v:string)=>setForm(f=>({...f,[k]:v}));
 return <main className="min-h-screen bg-[#FBFDFC] p-4 md:p-10"><div className="mx-auto max-w-2xl rounded-2xl bg-white p-6 md:p-8 shadow-sm border"><BrandLogo compact className="mb-6 w-[170px]" /><h1 className="text-2xl font-medium text-[#12352B]">Cadastro da empresa</h1><p className="mt-2 text-sm text-[#5E776D]">Crie o acesso da empresa. A compra será liberada após a aprovação e a definição dos preços.</p><form className="grid md:grid-cols-2 gap-4 mt-6" onSubmit={async e=>{e.preventDefault();setMessage("");await signup.mutateAsync(form)}}>
