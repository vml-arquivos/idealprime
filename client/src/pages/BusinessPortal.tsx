...
import { FileText, ShoppingCart, ClipboardList, CheckCircle2, Clock3, XCircle, Send, ArrowRight, AlertCircle } from "lucide-react";
...
export default function BusinessPortal() {
  ...
  const catalogCategories = useMemo(() => {
    return Array.from(new Set(catalogItems.map((item) => item.category_label?.trim()).filter(Boolean) as string[])).sort((a, b) => a.localeCompare(b, "pt-BR"));
  }, [catalogItems]);
  ...
  const selectedItems = useMemo(
    () => Object.entries(cart).filter(([, quantity]) => quantity > 0).map(([productId, quantity]) => ({ productId: Number(productId), quantity })),
    [cart],
  );

  const selectedCatalogItems = useMemo(
    () => selectedItems.map((selected) => ({ selected, item: catalogItems.find((item) => item.id === selected.productId) })).filter((entry) => entry.item),
    [selectedItems, catalogItems],
  );

  const selectedTotal = selectedCatalogItems.reduce((total, entry) => total + (entry.item?.price_cents || 0) * entry.selected.quantity, 0);
  const hasSelectionWithoutPrice = selectedCatalogItems.some((entry) => !hasCommercialPrice(Number(entry.item?.price_cents || 0)));
  ...
  const createOrder = trpc.b2b.createOrder.useMutation({
    onSuccess: async () => {
      toast.success("Pedido enviado para a Ideal Prime.");
      setCart({});
      await refreshPortal();
      setActiveTab("orders");
    },
    ...
    <main className="min-h-screen bg-[#f6f8f7] text-[#17352c]">
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 md:px-8 md:py-10">
        {me.isLoading ? (
          ...
          <section className="rounded-2xl border border-[#dcebe5] bg-white p-8 shadow-sm">
            <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-full bg-[#e8f1fb] text-[#098EC7]">
          ...
          <>
            ...
            <nav className="mt-6 flex flex-wrap gap-2 rounded-2xl border border-[#dcebe5] bg-white p-2 shadow-sm" aria-label="Área empresarial">
              {tabs.map(({ key, label, icon: Icon }) => (
                <button
                  key={key}
                  onClick={() => setActiveTab(key)}
                  className={`inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold transition-colors ${activeTab === key ? "bg-[#e8f1fb] text-[#098EC7]" : "text-[#5e776d] hover:bg-[#f1f8f5] hover:text-[#067c52]"}`}
                >
              ...
              <section className="mt-8">
                ...
                {catalogCategories.length > 0 && (
                  <div className="mt-4 flex flex-wrap gap-2">
                    <button onClick={() => setCategoryFilter(null)} className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition-colors ${categoryFilter === null ? "border-[#067c52] bg-[#067c52] text-white" : "border-[#cfe3da] bg-white text-[#5e776d] hover:border-[#067c52]"}`}>
                      Todos
                    </button>
                    {catalogCategories.map((categoryName) => (
                      <button key={categoryName} onClick={() => setCategoryFilter(categoryName)} className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition-colors ${categoryFilter === categoryName ? "border-[#067c52] bg-[#067c52] text-white" : "border-[#cfe3da] bg-white text-[#5e776d] hover:border-[#067c52]"}`}>
                        {categoryName}
                      </button>
                    ))}
                  </div>
                ...
                <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {items.map((product) => {
                    ...
                    return (
                      <article key={product.id} className="overflow-hidden rounded-2xl border border-[#dcebe5] bg-white shadow-sm transition-transform hover:-translate-y-0.5">
                        <div className="aspect-square bg-[#f1f8f5]">
                        ...
                        <div className="p-4">
                          ...
                          <div className="mt-3 text-lg font-semibold text-[#067c52]">{noPrice ? "Preço sob consulta" : money(product.price_cents)}</div>
                ...
                <div className="mt-8 grid gap-4 lg:grid-cols-[1fr_360px]">
                  <div className="rounded-2xl border border-[#dcebe5] bg-white p-5">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="text-sm text-[#5e776d]">{selectedItems.length} produto(s) {selectedItems.length > 0 ? `· estimativa ${money(selectedTotal)}` : "selecionado(s)"}</p>
                      </div>
                      <ShoppingCart className="h-5 w-5 text-[#098EC7]" />
                    </div>

                    {hasSelectionWithoutPrice && (
                      <div className="mt-4 flex gap-3 rounded-2xl border border-[#E8D8B8] bg-[#FFF8EA] p-4 text-sm text-[#7A5B1A]">
                        <AlertCircle className="mt-0.5 h-4 w-4 shrink-0" />
                        <p>Há item(ns) sem preço comercial definido. Para estes casos, utilize a cotação. O pedido direto fica liberado apenas quando todos os itens possuem preço.</p>
                      </div>
                    )}

                    <div className="mt-4 flex flex-wrap gap-3">
                      {canOrders && (
                        <Button disabled={!selectedItems.length || hasSelectionWithoutPrice || createOrder.isPending} onClick={() => createOrder.mutate({ items: selectedItems, paymentMethod: "MANUAL", idempotencyKey: crypto.randomUUID() })} className="gap-2 bg-[#067c52] hover:bg-[#056343]">
                          <Send className="h-4 w-4" />
                          Enviar pedido
                        </Button>
                      )}
                      {canQuotes && (
                        <Button disabled={!selectedItems.length || createQuote.isPending} variant="outline" onClick={() => setActiveTab("quotes")} className="gap-2 border-[#098EC7] text-[#098EC7] hover:bg-[#e8f1fb]">
                  ...
                  <div className="rounded-2xl border border-[#dcebe5] bg-[#f1f8f5] p-5">
                    <p className="text-sm font-semibold text-[#067c52]">Compra empresarial Prime</p>
                    <p className="mt-2 text-sm leading-6 text-[#5e776d]">Pedidos diretos reservam estoque. Cotações passam pela análise comercial antes da conversão em pedido. Produtos sem preço definido seguem exclusivamente por cotação.</p>
                  </div>
                </div>
              </section>
            )}
            ...
            {activeTab === "quotes" && canQuotes && (
              <section className="mt-8 grid gap-6 lg:grid-cols-[1fr_380px]">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#098EC7]">Negociação comercial</p>
                  <h2 className="mt-1 text-2xl font-semibold">Minhas cotações</h2>
                  <div className="mt-5 space-y-3">
                    {((quotes.data || []) as QuoteRow[]).map((quote) => (
                      <article key={quote.id} className="rounded-2xl border border-[#dcebe5] bg-white p-5 shadow-sm">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            <p className="font-semibold text-[#17352c]">{quote.quote_number}</p>
                            <p className="mt-1 text-xs text-[#5e776d]">{new Date(quote.created_at).toLocaleString("pt-BR")} · {quote.items?.length || 0} item(ns)</p>
                          </div>
                          <StatusBadge status={quote.status} />
                        </div>
                        <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                          <strong className="text-lg text-[#067c52]">{money(Number(quote.total_cents || 0))}</strong>
                          {quote.status === "APPROVED" && canOrders && (
                            <Button size="sm" onClick={() => convertQuote.mutate({ quoteId: quote.id, idempotencyKey: crypto.randomUUID() })} disabled={convertQuote.isPending} className="gap-2 bg-[#067c52] hover:bg-[#056343]">
                              Virar pedido <ArrowRight className="h-4 w-4" />
                            </Button>
                          )}
                        </div>
                      </article>
                    ))}
                    {!quotes.data?.length && <div className="rounded-2xl border border-dashed border-[#b8d9ca] bg-white p-10 text-center text-sm text-[#5e776d]">Você ainda não enviou uma cotação.</div>}
                  </div>
                </div>

                <div className="rounded-2xl border border-[#dcebe5] bg-white p-5 shadow-sm">
                  <h3 className="font-semibold">Nova cotação</h3>
                  <p className="mt-1 text-sm text-[#5e776d]">Volte ao catálogo para selecionar os produtos e descreva uma necessidade comercial.</p>
                  <Textarea className="mt-4 min-h-32" placeholder="Observações, prazo, condição ou contexto do pedido" value={notes} onChange={(event) => setNotes(event.target.value)} />
                  {canCatalog ? <Button className="mt-3 w-full gap-2 border-[#098EC7] text-[#098EC7]" variant="outline" onClick={() => setActiveTab("catalog")}>Selecionar produtos <ArrowRight className="h-4 w-4" /></Button> : <p className="mt-3 rounded-lg bg-[#f1f8f5] p-3 text-xs text-[#5e776d]">O catálogo não está liberado para esta conta.</p>}
                  {selectedItems.length > 0 && <Button className="mt-2 w-full gap-2 bg-[#098EC7] hover:bg-[#0678A8]" onClick={() => createQuote.mutate({ items: selectedItems, notes, idempotencyKey: crypto.randomUUID() })} disabled={createQuote.isPending}><FileText className="h-4 w-4" />Enviar seleção para cotação</Button>}
                </div>
              </section>
            )}
            ...
            {activeTab === "orders" && canHistory && (
              <section className="mt-8">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#098EC7]">Acompanhamento</p>
                <h2 className="mt-1 text-2xl font-semibold">Meus pedidos</h2>
                <div className="mt-5 space-y-3">
                  {((orders.data || []) as any[]).map((order) => (
                    <article key={order.id} className="flex flex-col justify-between gap-3 rounded-2xl border border-[#dcebe5] bg-white p-5 shadow-sm md:flex-row md:items-center">
                      <div>
                        <p className="font-semibold">{order.order_number}</p>
                        <p className="mt-1 text-xs text-[#5e776d]">{new Date(order.created_at).toLocaleString("pt-BR")}</p>
                      </div>
                      <div className="text-left md:text-right">
                        <p className="text-lg font-semibold text-[#067c52]">{money(Number(order.total_cents || 0))}</p>
                        <p className="text-xs text-[#5e776d]">{order.commercial_status} · {order.payment_status} · {order.fulfillment_status}</p>
                      </div>
                    </article>
                  ))}
