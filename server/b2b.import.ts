...
// Limite de linhas por arquivo: evita abuso (upload gigante prendendo a transação de
// importação — que faz `FOR UPDATE` por SKU — por tempo excessivo). Ver auditoria M3/A1.
const MAX_ROWS=5000;
...
export function parseImportBuffer(buffer:Buffer,filename:string){
 ...
 const normalized=rows.filter(r=>Object.values(r).some(v=>String(v).trim())).map(normalizeRow);
 if(normalized.length>MAX_ROWS)throw new Error(`Arquivo excede o limite de ${MAX_ROWS} linhas por importação`);
 return {rows:normalized,hash:crypto.createHash('sha256').update(buffer).digest('hex')};
}
