...
export const b2bQuotes = pgTable(
  ...
  (table) => [
    ...
    check("permupay_b2b_quote_subtotal_nonnegative", sql`${table.subtotalCents} >= 0`),
    check("permupay_b2b_quote_discount_nonnegative", sql`${table.discountCents} >= 0`),
    check("permupay_b2b_quote_freight_nonnegative", sql`${table.freightCents} >= 0`),
...
export const b2bQuoteItems = pgTable(
  "permupay_b2b_quote_items",
  {
    ...
    unitPriceCents: integer("unit_price_cents").notNull(),
    totalCents: integer("total_cents").notNull(),
    catalogUnitPriceCents: integer("catalog_unit_price_cents").notNull().default(0),
    quotedUnitPriceCents: integer("quoted_unit_price_cents").notNull().default(0),
    quotedTotalCents: integer("quoted_total_cents").notNull().default(0),
  },
  (table) => [
    check("permupay_b2b_quote_item_qty_positive", sql`${table.quantity} > 0`),
    check("permupay_b2b_quote_catalog_price_nonnegative", sql`${table.catalogUnitPriceCents} >= 0`),
    check("permupay_b2b_quote_quoted_price_nonnegative", sql`${table.quotedUnitPriceCents} >= 0`),
    check("permupay_b2b_quote_quoted_total_nonnegative", sql`${table.quotedTotalCents} >= 0`),
  ],
);
